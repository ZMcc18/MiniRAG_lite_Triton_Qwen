...
# 导入python的类型提示相关工具，前者表示既可以是某种类型也可是None
...
class Attention(nn.Module):
    ...
    def context_forward(
        ...
        # 1. 获取prefill阶段的cur_select_index，并更新kv cache张量
        # cur_select_index 是预先申请的全局显存块编号
        combined_kv = torch.cat([xk,xv], dim=-2) #(B, L ,2*num_kv_heads, head_dim)
        # 更新 kv_buffer, atten_info.kv_buffer[layer_index]
        # kv_buffer 是一个存储所有层KV缓存的列表/数组,layer_index 表示当前处理的Transformer层索引
        ...
        # 通过记录B_seq_len完成不等长序列的计算，也就是no pad
        output = flash_attention2_no_pad(
            xq, xk, xv,
            qk_scale,
            atten_info.b_start_loc, # 批次中每个请求的开始索引位置
            atten_info.b_seq_len, # 每个请求的序列长度
            atten_info.max_actual_seq_len,  # 最大实际序列长度
        )
        return output # (B*L,num_heads,head_dim)
    ...
    # 2. flashattention 计算: softmax(qk^t) * v
        output = flash_decoding(
            xq,
            # 分离kv缓存向量，前num_kv_heads是key，后num_kv_heads是value，目的是为了兼容GQA
