...
# 内核精度验证与性能比较函数封装
# =============================================================================
def run_flash_attention2_no_pad_benchmark(batch=4, n_heads=32, head_dim=128, max_seq_len_list=[1024, 2048, 4096]):
    """
    ...
    triton_output = flash_attention2_no_pad(q, k, v, sm_scale, b_start_loc, b_seq_len, max_seq_len)
    torch_output = standard_attention_prefill(q, k, v, b_start_loc, b_seq_len, sdpa=False)
    ...
    for seq_len in max_seq_len_list:
        ...
        # 构造 b_start_loc 和 b_seq_len (假设每个 batch 从 0 开始，序列长度均为 max_seq_len)
        b_start_loc = torch.tensor([0, seq_len, 2*seq_len, 3*seq_len], dtype=torch.int32, device="cuda") # batch = 4
